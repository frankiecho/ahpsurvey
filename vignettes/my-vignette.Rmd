...
```{r dpi=300,fig.cap="\\label{fig:figs}Maximum difference of between eigenvalue and mean aggregation", fig.height=4, fig.width=7}
cityahp <- city.df %>% 
...
```{r dpi = 300,fig.cap="\\label{fig:figs}Changes of aggregated weights based on quantile of data trimmed", fig.height=4, fig.width=7}
...
```{r, dpi = 300,fig.cap="\\label{fig:figs}Individual Priorities with respect to goal", fig.height=4, fig.width=7}
thres <- 0.1
dict <- c("cult" = "Culture", 
          "fam" = "Family", 
          "house" = "Housing", 
...
```{r dpi = 300, fig.height=4, fig.width=7, fig.cap="\\label{fig:figs}Consistency Ratios under different number of iterations with Harker's method"}
crmat %>% 
  as.data.frame() %>%
...
```{r dpi = 300, fig.height=4, fig.width=7, fig.cap="\\label{fig:figs}Individual priority weights with respect to goal (1 iteration)"}
it <- 1
thres <- 0.1
